...
class TestRegister:
    async def test_success(self, client):
        ...
        r = await client.post(
            "/auth/register",
            json={"email": email, "password": "Pass1234!", "fullname": "New User"},
        )
    ...
    async def test_invalid_email_returns_422(self, client):
        r = await client.post(
            "/auth/register", json={"email": "not-an-email", "password": "Pass1234!"}
        )
...
class TestLogin:
    async def test_success_returns_tokens(self, client, admin):
        r = await client.post(
            "/auth/login", json={"email": admin["email"], "password": "E2eTest1234!"}
        )
    ...
    async def test_wrong_password_returns_401(self, client, admin):
        r = await client.post(
            "/auth/login", json={"email": admin["email"], "password": "WrongPassword!"}
        )
    ...
    async def test_unknown_email_returns_401(self, client):
        r = await client.post(
            "/auth/login",
            json={"email": "nobody@nowhere.test", "password": "Pass1234!"},
        )
        assert r.status_code == 401
    ...
    async def test_inactive_user_returns_403(self, client, db):
        ...
        r = await client.post(
            "/auth/register",
            json={"email": email, "password": "Pass1234!", "fullname": "Inactive"},
        )
        assert r.status_code == 201
