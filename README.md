...
Backend
    ...
    nats-py: Асинхронный клиент для работы с брокером сообщений.

    Asyncpg & Redis-py: Высокоскоростные драйверы для работы с базами данных.

Infrastructure & Data

    NATS JetStream: Легковесная шина сообщений (брокер) для передачи сырых данных между сервисами.
...
Проект разделен на независимые слои:
    ...
    Ingestion Service: Принимает HTTP-трафик и мгновенно сбрасывает его в NATS (устойчивость к нагрузкам).
